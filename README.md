...
EDG 还提供实验性的 C Native 后端，可将受支持的 `.edg` 程序编译为本地可执行文件。它适合性能测试、嵌入式部署和运行时验证，目前仍属于实验功能。

```bash
python3 edg.py native examples/native_basic.edg -o /tmp/edg-native
/tmp/edg-native
```
...
编译器会生成 C 源码并调用本机 C 编译器。需要系统中可用的 `cc` 或兼容 C 编译器。Native 后端现在是唯一执行路径；类、字典、模块、闭包和完整标准库仍属于待实现的 Native 功能。
...
`edg_native.py` 是当前唯一的编译后端；`edg02.py` 仅保留 Native 编译所需的词法和表达式解析代码。
...
```bash
python3 -m py_compile edg.py src/edg02.py src/edg_native.py src/edg_native_runtime.py src/edg_hot.py src/spatial.py src/world.py
python3 edg.py examples/try_now.edg
...
1. 完善词法和语法错误位置提示
2. 将 `edg02` 的解析代码逐步并入 Native 编译器
3. 增加字符串、列表方法和标准库
4. 增加模块搜索路径和缓存管理
5. 补充单元测试、集成测试和基准测试
